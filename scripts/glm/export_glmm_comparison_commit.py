"""Commit-LL variant of export_glmm_comparison.py.

Overlay each model's cached GLMM coefficients on top of the human ("true")
GLMM fit, for every glmm_estimates_simulated_averaged.pkl under
data/POMDP_commit/.
...
For each model, writes:
  - figures/POMDP_commit/<task>/<rest-of-data-path>/glmm_betas_comparison_<task>_<horizon>.pdf/.svg/.png
    (mirrors the model's data/POMDP_commit/ subpath under figures/POMDP_commit/,
    instead of the flat ../GLM/ used by the non-commit script, since here each
    model gets its own figures subtree)
  - glmm_comparison_table.tex (next to the model's own data, in data_root)

Usage:
    uv run --directory notebooks export_glmm_comparison_commit.py
"""
...
if __name__ == "__main__":
    ...
    pkl_paths = sorted(
        glob.glob(
            os.path.join(
                project_root,
                "data/POMDP_commit/**/glmm_estimates_simulated_averaged.pkl",
            ),
            recursive=True,
        )
    )
    if not pkl_paths:
        print(
            "No glmm_estimates_simulated_averaged.pkl files found under "
            "data/POMDP_commit/"
        )
    ...
    for pkl_path in pkl_paths:
        ...
        # e.g. data/POMDP_commit/LB-XT-RPHCLUK/glm_vs_SBEXT-RPh----/glmm_estimates_simulated_averaged.pkl
        match = re.search(r"data/POMDP_commit/([^/]+)/", pkl_path)
        ...
        # Mirror data_root's data/POMDP_commit/... subpath under figures/POMDP_commit/...
        rel_data_root = os.path.relpath(
            data_root, os.path.join(project_root, "data/POMDP_commit")
        )
        model_figures_path = os.path.join(
            project_root, "figures/POMDP_commit", rel_data_root
        )
        ...
        # model_label is display-only (title/legend text and the LaTeX table's
        # column header); fname/data_root below keep using the raw `task` so
        # file paths still match what's actually on disk.
